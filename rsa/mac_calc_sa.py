...
# Modified by Eleisha Jackson, December 9, 2015
...
def get_values(output_dssp):
    ...
    RSAList = []  #This is is the list which will store the RSA values for each site
    ...
    for line in fileContents:
        if index < 28:  #This skips the first few lines that do not have the SA value data (Eleisha: Changed from 28)
            index = index + 1
            continue
            ...
            if AA != ( '!' or '*'):  #This takes out the missing gaps that dssp might put in
                if AA != 'X':
                    ...
                    RSAList.append(number / max_acc)  #This divides the SA value for that position by the amx SA value position for tha amino acid. This normalizes the values and gives us the Relative Solvent Accessability (RSA) value. We this appends this value to the list of RSA values
                    SAList.append(number)
                    AAList.append(AA)  #This appends the amino acid to the list
                    resList.append(res)
                    chainList.append(chain)
                elif AA == 'X':
                    RSAList.append(0)  #This divides the SA value for that position by the amx SA value position for tha amino acid. This normalizes the values and gives us the Relative Solvent Accessability (RSA) value. We this appends this value to the list of RSA values
                    SAList.append(0)
                    AAList.append(AA)  #This appends the amino acid to the list
                    resList.append(res)
                    chainList.append(chain)
    ...
    return (resList, chainList, AAList, SAList, RSAList)  #Return the RSA values and the SAList


def main():
    if len(sys.argv) != 3:
        print("\n\nUsage:\n\n     "+sys.argv[0]+" <output dssp ASA> <output SA>")
    else:
        #pdb_path = sys.argv[1]
        output_dssp = sys.argv[1]
        output_sa = sys.argv[2]     
        [res_List, chain_List, AA_List, SA, RSA] = get_values(output_dssp)
        #print res_List
        rsa_outputfile = open(output_sa, 'w')
        rsa_outputfile.write('Residue,Amino_Acid,Chain,SA,RSA\n')
        for i, aa in enumerate(AA_List):
            rsa_outputfile.write(str(res_List[i]) + ',' + str(aa) + ',' + chain_List[i] + ',' + str(SA[i]) + ',' + str(RSA[i]) + '\n')
        return
